...
const test = `describe('fizzbuzz test', () => {
    ...
    it('should work for multiples of 3', () => {
      expect(arr[2]).to.equal('fizz');
      expect(arr[5]).to.equal('fizz');
      expect(arr[8]).to.equal('fizz');
    });
    
    it('should work for multiples of 5', () => {
      expect(arr[4]).to.equal('buzz');
      expect(arr[9]).to.equal('buzz');
    });
    
    it('should work for multiples of 3 and 5', () => {
      expect(arr[14]).to.equal('fizzbuzz');
      expect(arr[29]).to.equal('fizzbuzz');
    });
  })
  `
